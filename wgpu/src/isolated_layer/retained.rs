...
impl<T: OutputTarget> Registry<T> {
    ...
    /// Leases the sole committed output when its complete key matches exactly,
    /// or reserves the slot for replacement. Content generations are compared
    /// for equality; no history of previously observed inputs is retained.
    pub(crate) fn lease_output(
        ...
        if let Some(entry) = self.entries.get_mut(&identity) {
            let _ = entry.observe_priority(request.priority(), frame);
    ...
    /// Returns output-slot metadata counts.
    #[cfg(test)]
    ...
    pub(crate) fn evict_to_bytes(&mut self, maximum: u64, frame: u64) -> EvictionOutcome<T> {
        ...
        debug_assert!(
            remaining_bytes <= maximum
                || self
                    .entries
                    .values()
                    .all(|entry| entry.committed.is_none() || entry.active.is_some()),
            "budget excess must belong only to ineligible active slots",
        );
    ...
    fn rejected_lookup(
        ...
        let _ = self
            .entries
            .get_mut(&identity)
            .is_some_and(|entry| entry.observe_priority(priority, frame));
...
fn rejected_lease<T>(miss: OutputMiss) -> OutputLease<T> {
    OutputLease {
        target: None,
        ticket: None,
        valid: false,
...
mod tests {
    ...
    #[test]
    fn content_equality_reuses_only_the_current_committed_output() {
        let surface = isolated_layer::SurfaceHandle::new();
        let a = isolated_layer::ContentChangeHandle::new();
        let b = isolated_layer::ContentChangeHandle::new();
        let c = isolated_layer::ContentChangeHandle::new();
        let context = context_at(10.0);
        let mut registry = Registry::<FakeTarget>::default();
        let inputs = [vec![&a, &b], vec![&a, &b], vec![&b], vec![&c], vec![&a, &b]];

        for (index, inputs) in inputs.into_iter().enumerate() {
            if index == 1 {
                let _ = a.mark_changed();
            }
            let frame = index as u64 + 1;
            let request = surface.cache_request(inputs);
            let output_key = key(&request, evidence(1), &context, false);
            let lease = registry.lease_output(&request, output_key.clone(), frame);
            assert!(lease.cacheable);
            assert!(!lease.valid);
            assert_eq!(
                lease.miss,
                Some(if index == 0 {
                    OutputMiss::UnknownIdentity
                } else {
                    OutputMiss::ContentChanged
                })
            );
            ...
            assert_eq!(
                ...
                if index == 0 {
                    ...
                    vec![FakeTarget::new(frame - 1)]
                }
            );

            // Equal evidence reuses this output, including after returning to
            // a dependency set observed earlier in the surface's lifetime.
            let hit = registry.lease_output(&request, output_key.clone(), frame);
            assert!(hit.valid);
            assert_eq!(hit.miss, None);
            assert_eq!(hit.target.as_ref().unwrap().id, frame);
            assert!(
                registry
                    .store_output(
                        hit.ticket.unwrap(),
        ...
        assert_eq!(registry.evict_to_bytes(0, 5).evicted.len(), 1);
        let entry = &registry.entries[&surface.identity()];
        assert!(entry.committed.is_none());
        assert!(entry.active.is_none());
        let request = surface.cache_request([&b]);
        let lease = registry.lease_output(&request, key(&request, evidence(1), &context, false), 6);
        assert_eq!(lease.miss, Some(OutputMiss::Targetless));
        assert!(lease.cacheable);
    }

    #[test]
    fn rejected_dependency_replacement_does_not_change_committed_stamp() {
        let surface = isolated_layer::SurfaceHandle::new();
        let a = isolated_layer::ContentChangeHandle::new();
        let b = isolated_layer::ContentChangeHandle::new();
        let context = context_at(10.0);
        let mut registry = Registry::<FakeTarget>::default();
        let request_a = surface.cache_request([&a]);
        let key_a = key(&request_a, evidence(1), &context, false);
        let initial = registry.lease_output(&request_a, key_a.clone(), 1);
        assert!(
            registry
                .store_output(
                    initial.ticket.unwrap(),
                    &request_a,
                    key_a.clone(),
                    1,
                    FakeTarget::new(1)
        ...
        let request_b = surface.cache_request([&b]);
        let key_b = key(&request_b, evidence(1), &context, false);
        let replacement = registry.lease_output(&request_b, key_b.clone(), 2);
        assert_eq!(replacement.miss, Some(OutputMiss::ContentChanged));
        let _ = b.mark_changed();
        let rejected = registry.store_output(
            replacement.ticket.unwrap(),
            &request_b,
            key_b.clone(),
            2,
            FakeTarget::new(2),
        ...
        let outdated = registry.lease_output(&request_b, key_b, 3);
        assert_eq!(outdated.miss, Some(OutputMiss::ContentChangedBeforeLookup));

        let hit = registry.lease_output(&request_a, key_a.clone(), 3);
        assert!(hit.valid);
        assert_eq!(hit.target.as_ref().unwrap().id, 1);
        assert!(
            registry
                .store_output(
                    hit.ticket.unwrap(),
    ...
    #[test]
    fn budget_respects_tiers_lru_and_active_replacements() {
        let context = context_at(10.0);
        let content = isolated_layer::ContentChangeHandle::new();
        let surfaces: Vec<_> = (0..4)
            .map(|_| isolated_layer::SurfaceHandle::new())
            .collect();
        let requests: Vec<_> = surfaces
            .iter()
            .enumerate()
            .map(|(index, surface)| {
                surface.cache_request_with(
                    if index == 0 {
                        isolated_layer::CacheResidencyPriority::Protected
                    } else {
                        isolated_layer::CacheResidencyPriority::Normal
        ...
        for (index, request) in requests.iter().enumerate() {
            let frame = index as u64 + 1;
            let key = key(request, evidence(1), &context, false);
            let lease = registry.lease_output(request, key.clone(), frame);
            assert!(
                registry
                    .store_output(
                        lease.ticket.unwrap(),
        ...
        // A keep-alive does not turn the oldest normal output into the newest.
        assert!(
            registry
                .keep_alive(&surfaces[1].cache_keep_alive(), 5)
                .found
        );
        let active = registry.lease_output(
            &requests[3],
            key(&requests[3], evidence(2), &context, false),
            5,
        );
        assert!(!active.valid);
        assert!(active.target.is_none());
        assert!(registry.evict_to_bytes(64, 5).evicted.is_empty());
        let outcome = registry.evict_to_bytes(32, 5);
        ...
        assert!(
            outcome
                .evicted
                .iter()
                .all(|entry| entry.tier == EvictionTier::Normal)
        );
        let outcome = registry.evict_to_bytes(0, 5);
        assert_eq!(outcome.remaining_bytes, 16);
        assert_eq!(outcome.evicted.len(), 1);
        assert_eq!(outcome.evicted[0].target.id, 1);
        assert_eq!(outcome.evicted[0].tier, EvictionTier::Protected);
        assert_eq!(registry.finish_frame(5).output_leases, 1);
        assert_eq!(registry.recover_abandoned(6).output_leases, 1);
        let outcome = registry.evict_to_bytes(0, 6);
        assert_eq!(outcome.remaining_bytes, 0);
        assert_eq!(outcome.evicted[0].target.id, 4);
        assert_eq!(registry.metadata_counts().outputs, 4);
        assert_eq!(registry.metadata_counts().resident_outputs, 0);
    }
