...
pub use super::{ScratchAllocator, ScratchError, ScratchTexture};
...
pub trait LayerEffect: Debug + Clone + PartialEq + MaybeSend + MaybeSync + 'static {
    ...
    /// unchanged.
    ///
    /// The default is conservatively `false` because an effect can inspect the
    /// absolute [`Context::represented_bounds`] and [`Context::content_bounds`].
...
///
/// Supplementary textures may use the renderer's [`ScratchAllocator`] for
/// application-scoped pooling and budget accounting. Independently allocated
/// resources remain private and unaccounted. In either case the pass owns its
/// internal dependencies and must initialize every sampled region and write its
/// final result into [`TextureViews::output`].
///
    ...
    P: Pass<E>,
{
    ...
    fn prepare(
        ...
        Box::new(Pass::prepare(
            self, effect, pipelines, device, queue, scratch, context, views,
        ))
    }
...
mod tests {
    ...
    impl<E: LayerEffect> Pass<E> for TestPass {
        ...
        fn prepare(
            ...
            _device: &wgpu::Device,
            _queue: &wgpu::Queue,
            _scratch: &mut ScratchAllocator<'_>,
            _context: &Context,
