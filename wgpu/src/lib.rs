...
impl Renderer {
    /// Returns isolated layer diagnostics for the most recently completed draw.
    ...
    pub fn draw(
        ...
        self.isolated_layers.begin_frame();
    ...
    fn draw_segmented(
        ...
        if needs_root_intermediate {
            let (mut root, hit) = self.isolated_layers.pool.lease_layer(
                &self.engine.device,
                self.engine.format,
                viewport.physical_size(),
            );
            self.isolated_layers
                .diagnostics
                .record_allocation(root.byte_size(), hit);
            root_context.set_backing_extent(root.extent);
            let root_text_viewport = root.prepare_text_viewport(
                &self.engine.device,
                &self.engine.text_pipeline,
                &self.engine.queue,
                viewport.physical_size(),
            ...
            self.prepare_sequence(encoder, sequence, &root_context, root_text_viewport);
            clear_view(
                encoder,
                &root.view,
            ...
            drop(composite);
            self.isolated_layers.pool.release(root);
        } else {
            ...
            self.prepare_sequence(encoder, sequence, &root_context, &mut root_text_viewport);
            clear_view(
                encoder,
                frame,
        ...
        for node in &mut sequence.0 {
            match node {
                ...
                isolated_layer::Node::Layer(node) => {
                    ...
                    self.isolated_layers
                        .diagnostics
                        .record_layer(prepared.output_valid, prepared.output_lease.is_some());
                    if !prepared.output_valid {
                        let text_viewport = prepared.targets[0].prepare_text_viewport(
                            &self.engine.device,
                            &self.engine.text_pipeline,
                            &self.engine.queue,
                            context.physical_viewport(),
                        );
                        self.prepare_sequence(encoder, &mut node.content, &context, text_viewport);
                    }
                }
                isolated_layer::Node::Effect(node) => {
                    ...
                    if !output_valid {
                        ...
                        for planned in &pass_plan.passes {
                            ...
                            let mut scratch = isolated_layer::ScratchAllocator::new(
                                &self.engine.device,
                                self.engine.format,
                                &mut self.isolated_layers.pool,
                                &mut self.isolated_layers.diagnostics,
                            );
    ...
    fn lease_target(&mut self, requested_size: Size<u32>) -> isolated_layer::Target {
        let (target, hit) = self.isolated_layers.pool.lease_layer(
            &self.engine.device,
            self.engine.format,
            requested_size,
        );
        self.isolated_layers
            .diagnostics
            .record_allocation(target.byte_size(), hit);
        target
    }
    ...
    fn lease_output(
        ...
        if let Some(miss) = lease.miss {
            self.isolated_layers
                .record_output_miss(cache_request.identity(), miss);
        }
            ...
            if let isolated_layer::Node::Leaf(leaf) = node {
                ...
                if let Some(prepared) = node.prepared.take() {
                    ...
                    for target in targets.into_iter().flatten() {
                        self.isolated_layers.pool.release(target);
                    }
                }
                ...
                if let Some(prepared) = node.prepared.take() {
                    ...
                    for pass in passes {
                        drop(pass.prepared);
                        isolated_layer::release_scratch(
                            &mut self.isolated_layers.pool,
                            pass.scratch,
                        );
                    }
