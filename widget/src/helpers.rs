...
#[cfg(feature = "wgpu")]
use crate::isolated_layer::{AlphaMask, DropShadow, DualKawaseBlur, GaussianBlur, IsolatedLayer};
use crate::keyed;
use crate::lazy::Lazy;
use crate::overlay;
...
///
/// Supply [`GaussianBlur::new`] with an outer logical-pixel radius. The dense
/// kernel grows with radius and display scale. Zero radius is an exact identity.
#[cfg(feature = "wgpu")]
pub fn blur_gaussian<'a, Message, Theme, Renderer>(
    content: impl Into<Element<'a, Message, Theme, Renderer>>,
...
where
    Renderer: crate::renderer::wgpu::isolated_layer::Renderer,
{
    IsolatedLayer::with_effect(content, settings)
}

/// Applies a Dual Kawase blur to `content`.
///
/// A scalar radius uses depth 3. Supply
/// `DualKawaseBlur::new(radius).pyramid_depth(depth)` to choose the depth.
/// Keep depth fixed for radius animation. Zero radius is an exact identity.
#[cfg(feature = "wgpu")]
pub fn blur_dual_kawase<'a, Message, Theme, Renderer>(
    content: impl Into<Element<'a, Message, Theme, Renderer>>,
    settings: impl Into<DualKawaseBlur>,
) -> IsolatedLayer<'a, Message, Theme, Renderer>
where
    Renderer: crate::renderer::wgpu::isolated_layer::Renderer,
{
    IsolatedLayer::with_effect(content, settings.into())
}
